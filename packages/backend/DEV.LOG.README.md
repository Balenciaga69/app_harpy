...
- 將生成工廠函數改成 class, 方便未來擴展與繼承

## _2025/12/26_

- 開始構思單元測試與 AI 品質檢查關卡機制
- 建立大量未實作空殼, 與單元測試. 讓 AI 品質檢查關卡能夠運作
- 添加大量指令, 從傳統乾淨代碼品質工法 轉為 AI 優先驅動

## v0.5.0

- 基礎各種 Domain 物件
- 遊戲初始化、物品敵人關卡生成、Repo Patterns、庫存系統、商店系統
- v0.4 版不兼容變更

## _2025/12/25_

- 研究 Go, C#, Ts 的開發哲學與精神。選型套用至本專案
- 建立庫存系統的領域物件與應用服務

## _2025/12/24_

- 構思啟動器與載入器以及遊戲初始化流程
- 研究不一致的抽象層級代碼異味, 業務流程協調、業務規則、對象創建、幫助類、訪問層、領域模型混合在一起的代碼異味

## _2025/12/23_

- 構思 更正大量命名把各種命稱生成器統一成 工廠 與 創造方法
- 思考 類別依賴注入 與 函數屬性傳遞 方法的取捨

## _2025/12/22_

- 構思 全局單例與 Prop Drilling 的機制的權衡
- 研究 Balatro 機制
- 研究 穿戴負載值取代裝備與聖物系統
...
- 重新定義何謂 關卡 "事件" 以及關卡節點配置
- 跟 GPT 詢問以下內容可行性(簡化縫合):
  - Slay the Spire, Monster Train 的 地圖分支與節點機制, 資源管理與戰利品節奏, 隨機事件
  - Risk of Rain 2 的 遺物系統
  - LoL TFT 奇遇與增幅裝置系統概念
  - Path of Exile 的 傳奇裝備模板, 沒有插槽概念
  - Path of Exile 的 頭盔上的附魔強化技能概念
  - Path of Exile 的 固定詞綴 (Affix) 與 修正項 (Modifier)
  - Path of Exile 的 裝備欄位系統
  - Skill Legends Royale 的 能量獲取與自動施放大招系統 (Ultimate)
  - Backpack Battles 的 異常狀態 (Aliment) 堆疊機制, 但不導入背包放置機制
  - Skill Legends Royale 的 Tick-based 1v1 自動戰鬥引擎
...
- 重新思考我目前轉換的架構有哪些維護上的不方便之處
- 與 AI 深度交流庫存系統應該有哪些東西
- 先把先前內容放到備用棄案區
- 開始暫停寫代碼, 重新規劃大方向企劃
- 放棄使用 AI 自動生成, 改用全程手刻後端遊戲邏輯, 或至少只能用極小範圍的 AI 生成輔助

## _2025/12/14_

- 在構想如果這個被轉換成前後端分離的應用, 哪些地方會需要重構
- 構思有沒有一套做法可以不論將其視為前端, 後端, 或後端模組都能正常溝通的使用方式
- Claude Sonnet 4.5 生成了庫存, 角色管理, 敵人生成系統
...
- 新增物品與大招系統機制
- 重構代碼, 讓每塊代碼更獨立, 更整齊, 未來新增功能或修改 bug 時會更有效率
