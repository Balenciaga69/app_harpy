...
- 這是一個不斷修改上下文狀態的邏輯處理器。輸入舊上下文，應用業務規則，輸出新上下文。上下文驅動的設計。
...
- 將整個 Run 的上下文拆分成多個子 JSON 欄位（如角色屬性 JSON、倉庫 JSON、商店 JSON、路線分支 JSON 等），然後將這些欄位存入 NoSQL 資料庫（遊戲核心 NoSQL）。每個 Run 對應一筆記錄，多欄位允許精準更新與查詢。
