...
Run 模組負責遊戲運行的整體管理，包括狀態轉換、協調器整合和運行生命週期。支援新運行創建、檢查點保存和狀態機驅動的遊戲流程。最後更新時間：2025-12-13。
...
Run 模組依賴 combat 模組的戰鬥執行，shop 模組的商店操作，character 模組的角色管理，以及 shared 模組的通用介面。Run 模組被 UI 模組依賴，用於遊戲運行控制，以及 gameplay 模組依賴，用於整體遊戲邏輯整合。
