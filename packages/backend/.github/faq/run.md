...
- Run 的狀態表明當前是甚麼階段, 例如你在戰鬥前準備時候, 不能去商店買東西(即戰鬥外).
- 狀態能讓系統知道目前能做甚麼事, 不能做甚麼事
- 當玩家前端關閉遊戲後, 他下次登入時也能從上次的狀態繼續遊戲.
- 舉例: 他如果在戰鬥前準備時斷線, 那麼只要他還沒按下確認戰鬥, 他下次登入時還是在戰鬥前準備狀態.
- 他如果已經按下確認戰鬥, 那麼他下次登入時就是在戰鬥中狀態.
- 如果他戰鬥後沒按下確認戰鬥結果, 那麼就是再次前端登入時還是在戰鬥後狀態.
- 分成以下狀態
  - 戰鬥外
  - 戰鬥前
  - 戰鬥中
