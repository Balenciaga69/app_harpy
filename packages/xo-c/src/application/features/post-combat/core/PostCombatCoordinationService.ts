...
export interface IPostCombatCoordinationService {
  claimRewardsAndAdvance(parameters: { selectedRewardIndexes: number[]; nextStageNumber: number }): Result<void, string>
}
export class PostCombatCoordinationService implements IPostCombatCoordinationService {
  ...
  claimRewardsAndAdvance(parameters: { selectedRewardIndexes: number[]; nextStageNumber: number }): Result<void, string> {
    ...
    const validateReward = this.postCombatContextHandler.validateRewardSelection(parameters.selectedRewardIndexes)
    ...
    const applyRewardResult = this.postCombatContextHandler.applyRewardsToCharacterAndStash(
      character,
      stash,
      parameters.selectedRewardIndexes
    )
    ...
    const advanceResult = this.runService.advanceToNextStage(parameters.nextStageNumber)
