### JWT & Auth

- JWT 與 Session 的選擇與 stateless 優劣?
- JWT 演算法選擇有何影響?
- JWT 的 payload 裡面可以放與不該放什麼資訊?
- JWT 應如何安全傳遞與存放?
- JWT 失效或被盜時，後端應如何處理?
- JWT Token 何時更新?可否同時攜帶多個 Token?
- JWT 不用 httpOnly 有何風險?
- 為何這個專案採用 Access Token 與 Refresh Token 的雙重 Token 機制?
- 如何控制多裝置驗證機制?
- Access Token 不查庫驗證的風險與效能取捨?
- 我何時才需要考慮導入 Refresh Token 的輪轉機制?

### Language Features

- AsyncLocalStorage 何時該使用? C# 中有類似的東西嗎?
- 在多實體的微服務情況下 NestJs中如何對某個端點限流?
- Passport.js 整套機制是怎麼運作的?

### Redis

- 為何我要用 Hash 而不是 String 來存取資料?
- 專案使用了 ioredis, keyv, cache-manager 這些各自的角色與關心的事有哪些?
