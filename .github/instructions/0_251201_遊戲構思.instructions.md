...
- CombatContext: 管理全域資源（EventBus、RNG、參戰實體、當前Tick），為各系統提供讀寫介面，並已重構為 ICombatContext 接口以解決循環依賴問題。
- EventBus: 戰鬥內事件總線，負責事件發佈與訂閱（tick、攻擊、死亡等），讓系統以事件驅動溝通。
- Character: 戰鬥中的實體（玩家或敵人），封裝屬性、裝備、遺物、效果與生命週期；對外暴露簡潔 API（屬性查詢、裝備、效果管理等）。目前已重構為使用 EquipmentManager、RelicManager 與 UltimateManager 來管理裝備、遺物與大絕招。
