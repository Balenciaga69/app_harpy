...
- 我有套件: zustand, mitt, seedRandom, nanoid,React(最終做好呈現) 等等
...
- 數據層 (依賴基礎層): 管理所有靜態與動態數據，並將其與業務邏輯分離。
  ...
  - Repository Pattern: 取代直接存取全域 Store，降低耦合(依賴注入)
- 核心邏輯層(依賴數據層)
...
- 戰鬥流程層(依賴數據層、核心邏輯層)
  ...
  - EventProcessor: 負責處理戰鬥中的各種事件。當事件發生時，它會通知 EffectManager，讓所有相關的 Effect 有機會響應事件並修改戰鬥流程。
    ...
    > 我希望是單向數據流 Store -> Repo -> CombatEngine -> Logger -> Store 將 Store 依賴封裝在 Repository 內
